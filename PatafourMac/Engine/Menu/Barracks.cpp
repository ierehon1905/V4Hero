    ...
    for(int i=0; i<v4core->savereader.invdata.items.size(); i++)
    {
        ...
        if(cur_item->category_id == activeCategory)
        {
            ...
            if(!ex)
            {
                ...
                tmp.r_outer.setSize({static_cast<float>(70.0*resRatioX), static_cast<float>(51.0*resRatioY)});
                tmp.r_outer.setFillColor(sf::Color(102,102,102,255));

                tmp.r_inner.setSize({static_cast<float>(46.0*resRatioX), static_cast<float>(46.0*resRatioY)});
...
void Barracks::Update(sf::RenderWindow &window, float fps, InputController& inputCtrl)
{
    if(isActive)
    {
        ...
        if(MenuMode)
        {
            ...
            for(int i=0; i<16; i++)
            {
                if(gridOffsetY*4 + i < inventory_boxes.size())
                {
                    if(inventory_boxes.size() > 0)
                    {
                        ...
                        {
                            inventory_boxes[curItem].r_highlight.setSize({static_cast<float>(70.0*resRatioX), static_cast<float>(51.0*resRatioY)});
                ...
                else
                {
                    ...
                    tmp_inv.r_outer.setSize({static_cast<float>(70.0*resRatioX), static_cast<float>(51.0*resRatioY)});
                    tmp_inv.r_outer.setFillColor(sf::Color(102,102,102,255));

                    tmp_inv.r_inner.setSize({static_cast<float>(46.0*resRatioX), static_cast<float>(46.0*resRatioY)});
                    ...
                    tmp_inv.r_highlight.setSize({static_cast<float>(70.0*resRatioX), static_cast<float>(51.0*resRatioY)});
            ...
            r_sel.setSize({static_cast<float>(70.0*resRatioX), static_cast<float>(51.0*resRatioY)});
