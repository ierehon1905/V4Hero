...
void AltarMenu::ReloadInventory()
{
    ...
    for(int i=0; i<v4core->savereader.invdata.items.size(); i++)
    {
        ...
        if(!ex)
        {
            ...
            tmp.r_outer.setSize({static_cast<float>(104.0*resRatioX), static_cast<float>(77.0*resRatioY)});
            tmp.r_outer.setFillColor(sf::Color(102,102,102,255));

            tmp.r_inner.setSize({static_cast<float>(72.0*resRatioX), static_cast<float>(72.0*resRatioY)});
    ...
    if((old_invboxes.size() > 0) || (!loadedSave)) ///if the invboxes are empty, don't highlight, unless it's a fresh save
    {
        ...
        for(int a=0; a<inventory_boxes.size(); a++)
        {
            ...
            if(highlight)
            {
                inventory_boxes[a].r_highlight.setSize({static_cast<float>(104.0*resRatioX), static_cast<float>(77.0*resRatioY)});
    ...
    if(isActive)
    {
        ...
        for(int i=0; i<24; i++)
        {
            ...
            else
            {
                ...
                tmp_inv.r_outer.setSize({static_cast<float>(104.0*resRatioX), static_cast<float>(77.0*resRatioY)});
                tmp_inv.r_outer.setFillColor(sf::Color(102,102,102,255));

                tmp_inv.r_inner.setSize({static_cast<float>(72.0*resRatioX), static_cast<float>(72.0*resRatioY)});
        ...
        r_sel.setSize({static_cast<float>(103.0*resRatioX), static_cast<float>(77.0*resRatioY)});
