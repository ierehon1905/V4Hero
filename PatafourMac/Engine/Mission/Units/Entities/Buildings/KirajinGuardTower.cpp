...
KirajinGuardTower::KirajinGuardTower()
{

}

void KirajinGuardTower::LoadConfig(Config *thisConfigs)
{
    /// all (normal) kacheeks have the same animations, so we load them from a hardcoded file
    AnimatedObject::LoadConfig(thisConfigs,"resources\\units\\entity\\kirajin_guardtower.p4a");
    ...
    s_broken.loadFromFile( resourcePath() + "resources/sfx/level/building_big_broken.ogg");
}

void KirajinGuardTower::parseAdditionalData(std::vector<std::string> additional_data)
{
...
{
    cout << "KirajinGuardTower::OnCollide" << endl;
